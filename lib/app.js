var app = {
    ...
    init: function(){
        ...
        Joint.paper("gradient-preview", 30, 82);
        GradientEditor.previewElement = Joint.paper().rect( 1, 1, 28, 80 ).attr( { fill: "white" } );
	...
	parse: function(cmd){
        ...
        exe: function(obj){
            var element, registeredElements;

            console.log("Executing: ", obj);
            switch (obj.cmd){
            case "new":
                if (obj.object == "joint"){
		    return Joint.dia.Joint(obj.from,
                                           obj.to,
                                           Joint.dia[obj.module][obj.type]
                                          ).registerForever(Joint.dia.registeredElements());
                } else {
                    return Joint.dia[obj.module][obj.object].create(obj.properties);
                }
                break;
            case "clear":
                Joint.resetPaper();
		...
		break;
            ...
            case "show-toolboxes":
                registeredElements = Joint.dia.registeredElements();
	        for (var i = 0, l = registeredElements.length; i < l; i++)
		    registeredElements[i].toggleToolbox(true);
                break;
            case "hide-toolboxes":
                registeredElements = Joint.dia.registeredElements();
	        for (var i = 0, l = registeredElements.length; i < l; i++)
		    registeredElements[i].toggleToolbox(false);
                break;
            case "open":
		try {
